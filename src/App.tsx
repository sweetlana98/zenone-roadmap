...
function OwnersPicker({ owners, onChange, editable, T }) {
  ...
  const addCustom = () => {
    ...
    if (!OWNER_COLORS[n]) {
      const p = [["#1e2a3a","#7dd3fc"],["#2a1a3a","#d8b4fe"],["#1a2a1a","#86efac"],["#2a2a1a","#fde68a"]];
      OWNER_COLORS[n] = p[Object.keys(OWNER_COLORS).length % p.length];
    }
  ...
  return (
    <div ref={ref} style={{position:"relative",flexShrink:0}}>
      <div data-trigger onClick={e=>{ e.stopPropagation(); setOpen(o=>!o); }} style={{display:"flex",gap:3,flexWrap:"wrap",alignItems:"center",cursor:"pointer",minWidth:32}}>
        {owners.length === 0
          ? <span style={{fontSize:10,color:T?T.text5:"#555",padding:"1px 6px",borderRadius:10,border:`0.5px dashed ${T?T.border2:"#333"}`}}>+ owner</span>
          : <OwnerTags owners={owners}/>}
      </div>
      {open && (
        <div style={{position:"fixed",background:"#1f1f1f",border:"0.5px solid #444",borderRadius:10,padding:8,zIndex:9999,minWidth:180,maxHeight:300,overflowY:"auto",display:"flex",flexDirection:"column",gap:3,boxShadow:"0 8px 24px rgba(0,0,0,.8)"}}
          ...
          {owners.length > 0 && (
            <div style={{display:"flex",flexDirection:"column",gap:2,paddingBottom:4,borderBottom:"0.5px solid #222"}}>
              {owners.map(o => {
                const [bg,text] = OWNER_COLORS[o] || ["#252525","#9ca3af"];
                return (
          ...
          {Object.keys(OWNER_COLORS).filter(o=>!owners.includes(o)).map(o => {
            ...
            return (
              <div key={o} style={{display:"flex",alignItems:"center",gap:4,borderRadius:6,padding:"3px 6px",border:"0.5px solid transparent"}}
                ...
                {confirmRemove === o
                  ? <span style={{display:"flex",gap:3,alignItems:"center"}}>
                      <span style={{fontSize:9,color:"#9ca3af"}}>Remove?</span>
                      <span onClick={()=>removeFromRoster(o)} style={{color:"#ef4444",fontSize:10,cursor:"pointer",fontWeight:600}}>Yes</span>
                      <span onClick={()=>setConfirmRemove(null)} style={{color:"#6b7280",fontSize:10,cursor:"pointer",marginLeft:3}}>No</span>
                    </span>
          ...
          <div style={{borderTop:"0.5px solid #2a2a2a",marginTop:4,paddingTop:6,display:"flex",gap:4}}>
            <input placeholder="Add name…" value={custom} onChange={e=>setCustom(e.target.value)}
              onKeyDown={e=>{ if(e.key==="Enter")addCustom(); e.stopPropagation(); }}
              onClick={e=>e.stopPropagation()}
              style={{flex:1,background:"#2a2a2a",border:"0.5px solid #444",borderRadius:4,color:"#e0e0e0",fontSize:10,padding:"4px 6px",outline:"none"}}/>
...
function FeatureCard({ feature, onUpdate, onDelete, onDragStart, onDragEnd, isDragging, onDragOverCard, dropPosition, editable, isPublic, T, darkMode }) {
  ...
  const cyclePhase = key => {
    const o = ["not-started","in-progress","done"];
    onUpdate(feature.id, { phases: {...feature.phases, [key]: o[(o.indexOf(feature.phases[key])+1)%o.length]} });
  };
      ...
      <div
        ...
        <div style={{background:T.bg4,padding:"9px 10px"}}>
          <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
            {editable && (
              <span onMouseDown={e=>{ e.stopPropagation(); setDragging(true); }} onMouseUp={()=>setDragging(false)} onMouseLeave={()=>setDragging(false)}
            ...
            <InlineEdit value={feature.name} onChange={v=>onUpdate(feature.id,{name:v})} editable={editable} T={T}
              style={{flex:1,minWidth:0,fontSize:13,fontWeight:600,color:T.text,lineHeight:1.4,wordBreak:"break-word",whiteSpace:"normal"}}/>
          ...
          <div style={{padding:"8px 10px 10px"}}>
            <div style={{display:"flex",gap:5,marginBottom:8,flexWrap:"wrap"}}>
              {[["product","Product"],["design","Design"],["eng","Eng"]].map(([key,lbl]) => {
                ...
                return (
                  <span key={key} onClick={!isPublic ? ()=>cyclePhase(key) : undefined}
                    style={{display:"inline-flex",alignItems:"center",background:c.bg,color:c.text,border:`0.5px solid ${c.border}`,borderRadius:4,padding:"2px 7px",fontSize:10,fontWeight:500,textDecoration:cfg.strike?"line-through":"none",cursor:isPublic?"default":"pointer",userSelect:"none",whiteSpace:"nowrap"}}>
                    {lbl}
                  </span>
                );
              })}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:3,marginBottom:6}}>
              {(feature.tasks||[]).map(task => (
                <div key={task.id} style={{display:"flex",alignItems:"flex-start",gap:6,background:T.bg5,borderRadius:6,padding:"5px 8px",border:`0.5px solid ${task.done?"#166534":T.border}`,textAlign:"left"}}>
                  <div onClick={!isPublic ? ()=>updateTask(task.id,{done:!task.done}) : undefined}
                    style={{width:14,height:14,borderRadius:3,border:`0.5px solid ${task.done?"#166534":T.border3}`,background:task.done?"#1a3a2a":"transparent",flexShrink:0,marginTop:3,cursor:isPublic?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
                    {task.done && <span style={{color:"#4ade80",fontSize:10,lineHeight:1}}>✓</span>}
                  </div>
                  {editable && !task.done ? (
                    <textarea value={task.name}
                      onChange={e=>{ updateTask(task.id,{name:e.target.value}); e.target.style.height="auto"; e.target.style.height=e.target.scrollHeight+"px"; }}
                    ...
                    <span style={{flex:1,fontSize:11,color:task.done?T.text4:T.text3,lineHeight:1.5,textDecoration:task.done?"line-through":"none",opacity:task.done?0.6:1,textAlign:"left",wordBreak:"break-word",whiteSpace:"normal"}}>
                      {task.name || <span style={{opacity:.3}}>Task description</span>}
                    </span>
                  )}
              ...
              {editable && (
                <button onClick={addTask} style={{background:"transparent",border:`0.5px dashed ${T.border2}`,borderRadius:6,color:T.text5,fontSize:10,padding:"4px 8px",cursor:"pointer",textAlign:"left"}}>+ Add task</button>
              )}
            </div>
            <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",paddingTop:4,borderTop:`0.5px solid ${T.border}`}}>
              {editable ? (
                <>
                  ...
                  <input type="number" min="0" value={feature.estNum||""} onChange={e=>onUpdate(feature.id,{estNum:e.target.value})} placeholder="–"
                    style={{width:36,background:T.inputBg,border:`0.5px solid ${T.border2}`,borderRadius:4,color:T.text4,fontSize:10,padding:"2px 4px",outline:"none",textAlign:"center"}}/>
                  <select value={feature.estUnit||"days"} onChange={e=>onUpdate(feature.id,{estUnit:e.target.value})}
                    style={{background:T.inputBg,border:`0.5px solid ${T.border2}`,borderRadius:4,color:T.text4,fontSize:10,padding:"2px 4px",outline:"none",cursor:"pointer"}}>
    ...
    <div
      ...
      <div style={{marginBottom:12}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:6}}>
          {editable && (
            <span onMouseDown={e=>{ e.stopPropagation(); setSprintDragging(true); }} onMouseUp={()=>setSprintDragging(false)} onMouseLeave={()=>setSprintDragging(false)}
              style={{color:T.text5,fontSize:13,cursor:"grab",flexShrink:0,userSelect:"none",marginTop:2,padding:"0 2px"}}>⠿</span>
          )}
          <div style={{flex:1}}>
      ...
      {editable && (
        <button onClick={()=>onAddFeature(sprint.id)} style={{width:"100%",background:"transparent",border:`0.5px dashed ${T.border}`,borderRadius:8,color:T.text5,fontSize:11,padding:"7px",cursor:"pointer",marginTop:4}}>+ New feature</button>
      )}
    </div>
  );
}
...
export default function App() {
        ...
        const fixed = {
          ...
          sprints: remote.sprints.map(s => ({
            ...
            features: s.features.map(f => {
              const ff = seenIds.has(f.id) ? {...f, id:newId()} : f;
              seenIds.add(ff.id);
              const tasks = (ff.tasks||[]).map(t => {
                if (seenIds.has(t.id)) { const nt={...t,id:newId()}; seenIds.add(nt.id); return nt; }
  ...
  const handleDrop = useCallback(e => {
    ...
    upd(prev => {
      ...
      let sprints = prev.sprints.map(s => {
        const f = s.features.find(f => f.id===draggingId);
        if (f) { moved=f; return {...s, features:s.features.filter(f => f.id!==draggingId)}; }
        return s;
      });
  ...
  const handleSprintDragOver = useCallback((sid, e) => {
    if (!e || !e.currentTarget) return;
    const rect = e.currentTarget.getBoundingClientRect();
    setOverSprintId(sid);
    setSprintDropSide(e.clientX < rect.left+rect.width/2 ? "left" : "right");
  }, []);
  ...
  const handleSprintDrop = useCallback((targetId, e) => {
    ...
    upd(prev => {
      ...
      sprints.splice(side==="left" ? toIdx : toIdx+1, 0, moved);
      return {...prev, sprints};
    });
  ...
  if (!data) return (
    <div style={{background:T.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",color:T.text5,fontFamily:"system-ui"}}>
      Loading roadmap…
    </div>
  );

  const filtered = {...data, sprints:data.sprints.map(s => ({...s, features:s.features.filter(f => {
    if (filter==="xl-l") return f.size==="XL"||f.size==="L";
    if (filter==="m-s") return f.size==="M"||f.size==="S";
    return true;
  })}))};

  const fBtn = (v,l) => (
    <button key={v} onClick={()=>setFilter(v)}
      style={{padding:"4px 12px",borderRadius:20,fontSize:11,cursor:"pointer",border:"0.5px solid",background:filter===v?T.bg5:"transparent",color:filter===v?T.text:T.text5,borderColor:filter===v?T.border3:T.border}}>
      {l}
    </button>
  );
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          {!isPublic && filter==="all" && (
            <button onClick={()=>setEditMode(m=>!m)}
              style={{padding:"4px 14px",borderRadius:20,fontSize:11,cursor:"pointer",border:`0.5px solid ${editMode?"#4ade80":T.border}`,background:editMode?"#1a3a2a":"transparent",color:editMode?"#4ade80":T.text5,transition:"all .2s"}}>
              {editMode ? "✓ Editing" : "Edit"}
            </button>
          )}
          {editable && (
            <button onClick={addSprint} style={{padding:"4px 12px",borderRadius:20,fontSize:11,cursor:"pointer",border:`0.5px solid ${T.border}`,background:"transparent",color:T.text5}}>+ Sprint</button>
          )}
          {!isPublic && (
            <button onClick={copyPublicLink}
              style={{padding:"4px 12px",borderRadius:20,fontSize:11,cursor:"pointer",border:`0.5px solid ${copied?"#4ade80":T.border}`,background:copied?"#1a3a2a":"transparent",color:copied?"#4ade80":T.text5,transition:"all .2s"}}>
              {copied ? "✓ Copied!" : "Share ↗"}
            </button>
          )}
          <button onClick={toggleDark} title="Toggle light/dark"
            style={{padding:"4px 10px",borderRadius:20,fontSize:12,cursor:"pointer",border:`0.5px solid ${T.border}`,background:"transparent",color:T.text5}}>
            {darkMode ? "☀️" : "🌙"}
          </button>
      ...
      <div ref={boardRef}
        ...
        {filtered.sprints.map(s => (
          <SprintCol key={s.id} sprint={s}
            ...
            onDragOverCard={handleDragOverCard} onDragOverEmpty={handleDragOverEmpty}
            editable={editable}
            onSprintDragStart={handleSprintDragStart} onSprintDragOver={handleSprintDragOver}
            onSprintDrop={handleSprintDrop} onSprintDragEnd={handleSprintDragEnd}
            isSprintDragging={draggingSprintId===s.id} colWidth={colWidth}
            isPublic={isPublic} isSprintDropTarget={overSprintId===s.id} sprintDropSide={sprintDropSide}
