        ...
        {!collapsed&&<div style={{padding:"8px 10px 10px"}}>
          <div style={{display:"flex",gap:5,marginBottom:8,flexWrap:"wrap"}}>
            {[["product","Product"],["design","Design"],["eng","Eng"]].map(([key,lbl])=>{
              ...
              return <span key={key} onClick={!isPublic?()=>cyclePhase(key):undefined} style={{display:"inline-flex",alignItems:"center",background:c.bg,color:c.text,border:`0.5px solid ${c.border}`,borderRadius:4,padding:"2px 7px",fontSize:10,fontWeight:500,textDecoration:c.strike?"line-through":"none",cursor:isPublic?"default":"pointer",userSelect:"none",whiteSpace:"nowrap"}}>{lbl}</span>;
            })}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:3,marginBottom:6}}>
            {(feature.tasks||[]).map(task=>(
              <div key={task.id} style={{display:"flex",alignItems:"flex-start",gap:6,background:"#242424",borderRadius:6,padding:"5px 8px",border:`0.5px solid ${task.done?"#166534":"#2d2d2d"}`,textAlign:"left"}}>
                <div onClick={!isPublic?()=>updateTask(task.id,{done:!task.done}):undefined} style={{width:14,height:14,borderRadius:3,border:`0.5px solid ${task.done?"#166534":"#444"}`,background:task.done?"#1a3a2a":"transparent",flexShrink:0,marginTop:3,cursor:isPublic?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
...
function SprintCol({ sprint, onUpdateFeature, onDeleteFeature, onAddFeature, onUpdateSprint, onDeleteSprint, onDragStart, onDragEnd, draggingId, dropTarget, onDragOverCard, onDragOverEmpty, editable, onSprintDragStart, onSprintDragOver, onSprintDrop, onSprintDragEnd, isSprintDragging, colWidth, isPublic, isSprintDropTarget, sprintDropSide }) {
  ...
  return (
    <div
      ...
      style={{flexShrink:0,width:colWidth,background:"#141414",borderRadius:12,padding:12,border:`0.5px solid ${isSprintDragging?"#534AB7":"#2a2a2a"}`,boxSizing:"border-box",opacity:isSprintDragging?0.4:1,transition:"opacity .15s, width .1s",position:"relative",
      ...
      {(sprint.features||[]).map(f=>{
        ...
        return <FeatureCard key={f.id} feature={f} onUpdate={onUpdateFeature} onDelete={onDeleteFeature} onDragStart={onDragStart} onDragEnd={onDragEnd} isDragging={draggingId===f.id} onDragOverCard={(fid,p)=>onDragOverCard(sprint.id,fid,p)} dropPosition={pos} editable={editable} isPublic={isPublic}/>;
      })}
...
export default function App() {
  ...
  const handleSprintDrop = useCallback((targetId, e) => {
    ...
    upd(prev => {
      ...
      let toIdx = sprints.findIndex(s => s.id === targetId);
      const [moved] = sprints.splice(fromIdx, 1);
      toIdx = sprints.findIndex(s => s.id === targetId);
    ...
    <div onDragOver={e=>{if(editable)e.preventDefault();}} onDrop={handleDrop}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          {!isPublic&&filter==="all"&&<button onClick={()=>setEditMode(m=>!m)} style={{padding:"4px 14px",borderRadius:20,fontSize:11,cursor:"pointer",border:`0.5px solid ${editMode?"#4ade80":"#2a2a2a"}`,background:editMode?"#1a3a2a":"transparent",color:editMode?"#4ade80":"#555",transition:"all .2s"}}>{editMode?"✓ Editing":"Edit"}</button>}
          ...
          {!isPublic&&<button onClick={copyPublicLink} style={{padding:"4px 12px",borderRadius:20,fontSize:11,cursor:"pointer",border:`0.5px solid ${copied?"#4ade80":"#2a2a2a"}`,background:copied?"#1a3a2a":"transparent",color:copied?"#4ade80":"#555",transition:"all .2s"}}>{copied?"✓ Copied!":"Share ↗"}</button>}
